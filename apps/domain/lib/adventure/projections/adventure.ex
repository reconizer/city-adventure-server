defmodule Domain.Adventure.Projections.Adventure do
  ...
  def get_adventure_by_id(%{id: adventure_id}, %{id: owner_id}) do
    from(adventure in Models.Adventure,
      ...
      left_join: user_ranking in Models.UserRanking, on: [adventure_id: adventure.id], limit: 5,
      select: %{
        id: adventure.id,
        description: adventure.description,
        name: adventure.name,
        min_time: fragment("EXTRACT(EPOCH FROM ?::time)", adventure.min_time),
        max_time: fragment("EXTRACT(EPOCH FROM ?::time)", adventure.max_time),
